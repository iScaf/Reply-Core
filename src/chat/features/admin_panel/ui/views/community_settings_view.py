...
class CommunitySettingsView(BaseTableView):
    ...
    def _get_item_by_id(self, item_id: str) -> Optional[Any]:
        ...
        try:
            ...
            # 查询新的表结构：community_settings.documents，使用 id 列
            cursor.execute(
                "SELECT * FROM community_settings.documents WHERE id = %s",
                (item_id,),
            )
    ...
    async def _build_list_embed(self) -> discord.Embed:
        ...
        try:
            ...
            if self.search_mode:
                ...
                embed = discord.Embed(
                    title=f"搜索社区设定 (关键词: '{self.search_keyword}')",
                    color=discord.Color.gold(),
                )
            else:
                # 查询新的表结构：community_settings.documents
                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.documents"
                )
                count_result = cursor.fetchone()
                total_rows = count_result["count"] if count_result else 0

                # 查询 chunks 表的 embedding 统计
                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks"
                )
                chunks_result = cursor.fetchone()
                total_chunks = chunks_result["count"] if chunks_result else 0

                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks WHERE bge_embedding IS NOT NULL"
                )
                bge_result = cursor.fetchone()
                bge_count = bge_result["count"] if bge_result else 0

                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks WHERE qwen_embedding IS NOT NULL"
                )
                ...
                cursor.execute(
                    "SELECT * FROM community_settings.documents ORDER BY created_at DESC, id DESC LIMIT %s OFFSET %s",
                    (self.items_per_page, offset),
                )
                page_items = cursor.fetchall()
                self.current_list_items = page_items
                embed = discord.Embed(
                    title="浏览：社区设定", color=discord.Color.green()
                )
    ...
    async def _build_detail_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"查看详情: {title}",
            description=f"表: `community_settings.documents` | ID: `#{self.current_item_id}`",
        ...
        if conn:
            try:
                ...
                # 查询该文档的 chunks 总数
                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks WHERE document_id = %s",
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks WHERE document_id = %s AND bge_embedding IS NOT NULL",
                    (self.current_item_id,),
                )
                bge_result = cursor.fetchone()
                bge_count = bge_result["count"] if bge_result else 0
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM community_settings.chunks WHERE document_id = %s AND qwen_embedding IS NOT NULL",
                    (self.current_item_id,),
                )
                qwen_result = cursor.fetchone()
                qwen_count = qwen_result["count"] if qwen_result else 0
