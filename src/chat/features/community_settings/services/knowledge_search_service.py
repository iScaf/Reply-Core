...
class KnowledgeSearchService:
    ...
    def __init__(self):
        ...
        # 从配置中加载参数
        self.config = {
            "TOP_K_VECTOR": chat_config.COMMUNITY_SETTINGS_RAG_CONFIG.get("TOP_K_VECTOR", 10),
            "TOP_K_FTS": chat_config.COMMUNITY_SETTINGS_RAG_CONFIG.get("TOP_K_FTS", 10),
            "RRF_K": chat_config.COMMUNITY_SETTINGS_RAG_CONFIG.get("RRF_K", 60),
            "HYBRID_SEARCH_FINAL_K": chat_config.COMMUNITY_SETTINGS_RAG_CONFIG.get(
    ...
    async def _hybrid_search_chunks(
        ...
        """
        在 community_settings.chunks 表中执行混合搜索（向量 + BM25 + RRF 融合），
        并返回融合排序后的 chunk 结果。
        """
        ...
        log.info(
            f"[社区设定搜索配置] Embedding模型: {embedding_model} | 搜索模式: 混合搜索 (向量 + BM25) | "
        ...
        sql_query = text(
            ...
            WITH semantic_search AS (
                -- 社区设定向量搜索
                SELECT
                    'community_settings' as source_table,
                    document_id,
                    chunk_text,
                    RANK() OVER (ORDER BY {embedding_col} <=> :query_vector) as rank
                FROM community_settings.chunks
                ORDER BY {embedding_col} <=> :query_vector
                LIMIT :top_k_vector
            ),
            keyword_search AS (
                -- 社区设定 BM25 搜索 (使用 paradedb.score)
                SELECT
                    'community_settings' as source_table,
    ...
    async def search(self, query: str) -> List[Dict[str, Any]]:
        ...
        """
        log.info(f"收到社区设定混合搜索请求: '{query}'")

        # 检查是否启用了向量功能
        if not is_vector_enabled():
            log.info("[社区设定搜索] 向量功能未启用，跳过搜索")
            return []
        ...
        if not search_results:
            log.info(f"社区设定混合搜索未找到 '{query}' 的相关文档。")
            return []

        # 转换为 community_settings_service 期望的格式
