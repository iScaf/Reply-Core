...
class IncrementalRAGService:
    ...
    async def _process_single_entry_to_parade(self, entry: Dict[str, Any]) -> bool:
        """
        处理单个社区设定条目，生成嵌入并添加到 Parade DB 向量数据库
        ...
        try:
            ...
            # 首先删除旧的向量数据
            await self._delete_entry_from_parade(entry_id)
            ...
            try:
                for chunk_index, chunk_content in enumerate(chunks):
                    ...
                    # 插入到 community_settings.chunks 表（双模型向量双写）
                    cursor.execute(
                        """
                        INSERT INTO community_settings.chunks
    ...
    async def _delete_entry_from_parade(self, entry_id: str) -> bool:
        """
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM community_settings.chunks WHERE document_id = %s",
                (entry_id,),
            )
            log.info(
                f"从 community_settings.chunks 表中删除了与 document_id {entry_id} 相关的所有文档块"
            )
    ...
    async def process_setting_entry(self, entry_id: str) -> bool:
        """
        处理单个社区设定条目，将其添加到向量数据库
        ...
        """
        if not self.is_ready():
            log.info("RAG功能未启用：未配置API密钥，跳过社区设定向量化。")
            return False

        log.debug(f"尝试处理社区设定条目: {entry_id}")
        # 从数据库获取通用知识条目
        entry_data = self._get_setting_entry_data(entry_id)
        if not entry_data:
            log.error(f"无法找到社区设定条目: {entry_id}")
            return False
        log.debug(f"成功获取社区设定条目数据: {entry_id}")

        # 构建RAG条目格式
        rag_entry = self._build_rag_entry_from_setting(entry_data)
        log.debug(f"为社区设定条目 {entry_id} 构建RAG条目: {rag_entry.get('id')}")

        # 处理并添加到 Parade DB 向量数据库
        success = await self._process_single_entry_to_parade(rag_entry)

        if success:
            log.info(f"成功将社区设定条目 {entry_id} 添加到向量数据库")
        else:
            log.error(f"处理社区设定条目 {entry_id} 时失败")

        return success

    def _get_setting_entry_data(self, entry_id: str) -> Dict[str, Any] | None:
        conn = self._get_parade_connection()
        if not conn:
            return None
        ...
        try:
            ...
            cursor.execute(
                ...
                FROM community_settings.documents
                WHERE id = %s
                """,
                (entry_id,),
            )
            ...
            if entry_row:
                ...
                log.debug(f"从 Parade DB 获取社区设定条目 {entry_id} 成功")
                ...
                if full_text:
                    try:
                        ...
                        log.warning(
                            f"解析 community_settings.documents #{entry_id} 的 full_text 失败: {e}"
                        )
                ...
                if source_metadata and not entry_dict.get("content"):
                    try:
                        ...
                        log.warning(
                            f"解析 community_settings.documents #{entry_id} 的 source_metadata 失败: {e}"
                        )
                        entry_dict["content"] = {}
            ...
            log.error(f"从 Parade DB 获取社区设定条目数据时出错: {e}", exc_info=True)
        finally:
            if cursor:
                cursor.close()
    ...
    def _build_rag_entry_from_setting(
        ...
        log.debug(f"构建的社区设定 RAG 条目: {rag_entry['id']}")
        return rag_entry
    ...
    async def delete_entry(self, entry_id: str) -> bool:
        ...
        try:
            ...
            # 从 community_settings.chunks 表中删除
            success = await self._delete_entry_from_parade(entry_id)
