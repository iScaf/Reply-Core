    ...
    async def generate_reply_stream(
        ...
        try:
            ...
            provider = ai_service.get_provider_for_model(
                *ai_service.parse_model_id(ai_service._get_default_model())
            )
        except Exception:
            available = False
        if not available or provider is None:
            yield (
                "degraded",
                self._degraded(citations, "AI Provider 未配置或不可用", started),
            )
            return
        if not hasattr(provider, "stream_chat"):
            yield (
                "degraded",
                self._degraded(citations, "当前 Provider 不支持流式输出", started),
            )
            return
        ...
        try:
            for round_no in range(1, TOOL_ITERATIONS + 1):
                ...
                async for ev in provider.stream_chat(
                    conversation, config=config, tools=tools, model=None
                ):
                    ev_type = ev["type"]
            ...
            if not reply:
                yield (
                    "degraded",
                    self._degraded(citations, "AI 返回了空回复", started),
                )
                return
            model_used = getattr(provider, "default_model", None)
            final_payload = {
                "reply": reply,
                "model": model_used,
                "citations": citations,
            ...
            await self._save_history(message, reply, final_payload)
            ...
            if content_buf.strip():
                ...
                await self._save_history(message, content_buf.strip(), final_payload)
                yield ("final", final_payload)
            else:
                yield ("error", {"message": f"AI 调用失败: {e}"})

    async def _save_history(
        self, message: str, reply: str, payload: Dict[str, Any]
    ) -> None:
        """持久化一问一答（user + assistant 两条消息）。失败仅记日志。"""
        try:
            ...
            log.warning(f"[Web 问答] 聊天记录保存失败: {e}")
